...
function findDiacriticPosition(key: string, core: Core): number {
  console.log("working on " + key)

  // range to check: -15 to 15

  const shape = (<path d={core.shape} />) as SVGPathElement
  svg.append(shape)

  const box = getBBox(shape)

  for (let x = (box.x + box.width) * 1e4; x > 1e4 * box.x; x -= 1) {
...
for (const key in CORES) {
  // @ts-ignore
  CORES[key].diacritic = findDiacriticPosition(key, CORES[key])
}
